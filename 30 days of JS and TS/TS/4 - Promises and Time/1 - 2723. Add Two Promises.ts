/*

https://leetcode.com/problems/add-two-promises/?envType=study-plan-v2&envId=30-days-of-javascript

2723. Add Two Promises

Given two promises promise1 and promise2, return a new promise. promise1 and promise2 will both resolve with a number. The returned promise should resolve with the sum of the two numbers.
 

Example 1:

Input: 
promise1 = new Promise(resolve => setTimeout(() => resolve(2), 20)), 
promise2 = new Promise(resolve => setTimeout(() => resolve(5), 60))
Output: 7
Explanation: The two input promises resolve with the values of 2 and 5 respectively. The returned promise should resolve with a value of 2 + 5 = 7. The time the returned promise resolves is not judged for this problem.
Example 2:

Input: 
promise1 = new Promise(resolve => setTimeout(() => resolve(10), 50)), 
promise2 = new Promise(resolve => setTimeout(() => resolve(-12), 30))
Output: -2
Explanation: The two input promises resolve with the values of 10 and -12 respectively. The returned promise should resolve with a value of 10 + -12 = -2.
 

Constraints:

promise1 and promise2 are promises that resolve with a number

*/

type P = Promise<number>;

async function addTwoPromises(promise1: P, promise2: P): P {
    const [a, b] = await Promise.all([promise1, promise2]);
    return a + b;
}

async function addTwoPromises1(promise1: P, promise2: P): P {
    try {
        let val1 = await promise1;
        let val2 = await promise2;
        return val1 + val2;
    } catch (e) {
        console.log(e);
        throw e;
    }
}

async function addTwoPromises3(promise1: P, promise2: P): P {
    return Promise.all([promise1, promise2]).then(
        ([val1, val2]) => val1 + val2
    );
}

async function addTwoPromises4(promise1: P, promise2: P): P {
    return promise1.then((val1) => {
        return promise2.then((val2) => {
            return val1 + val2;
        });
    });
}

async function addTwoPromises5(
    promise1: Promise<number>,
    promise2: Promise<number>
): Promise<number> {
    return promise1
        .then((val1) => {
            return promise2
                .then((val2) => {
                    return val1 + val2;
                })
                .catch((error) => {
                    console.error("Error in promise2: ", error);
                    throw error; // Re-throw the error to ensure it's not silently swallowed
                });
        })
        .catch((error) => {
            console.error("Error in promise1: ", error);
            throw error; // Re-throw the error to ensure it's not silently swallowed
        });
}

/**
 * addTwoPromises(Promise.resolve(2), Promise.resolve(2))
 *   .then(console.log); // 4
 */
